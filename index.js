...
app.post('/login', async function (req, res) {
    ...
    if (!checkUsername) {
        ...
        if (isPasswordMatch) {
            const token = jwt.sign({ id: checkUsername._id, name: checkUsername.fname, email: checkUsername.email }, process.env.SECRET_KEY);
            res.send({ token: token, data: checkUsername })
        } else {
            res.send(false)
        }
    }
